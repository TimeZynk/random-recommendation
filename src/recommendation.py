...
def fetch_work_hour_templates(url, headers, contracts):
    ...
    for contract in contracts:
        response_wh = requests.request("GET", url + '/work-hours-templates', headers=headers, params={"id": contract['template-id']})
...
def week_start_end(shift_time):
    ...
    week_end_real = dt(week_end.year, week_end.month, week_end.day) - timedelta(microseconds = 1)
        ...
        for user, work_hours_list in work_hours_data.items():
            for wh in work_hours_list:
                ...
                if shift_start >= wh['start-date'] and 'end-date' not in wh.keys():
                    intersection = (max(week_start, wh['start-date']), week_end)
                # Again not considering the situation of a shift last passing the end-date of a contract.
                    ...
                    intersection = (max(week_start, wh['start-date']), min(week_end, wh['end-date']))
                else:
                    continue
                ...
                time_diff = map(lambda x: min(dt.strptime(x['end'], fmt), week_end) - max(dt.strptime(x['start'], fmt), week_start), booked_shifts)
...
def fetch_no_work_hrs(qsse, url, headers):
    response = requests.request("GET", url + '/employment-contracts', headers=headers, params={"archived": "null"})
...
def recommend_and_return():
    ...
    excluded_users_list = list(map(lambda x,y,z,a: x.union(y,z,a), busy_users_list, unavailable_list, ineligible_users_list, no_work_hrs_list))
